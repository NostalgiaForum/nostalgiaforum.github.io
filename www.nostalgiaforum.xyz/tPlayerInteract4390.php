...
class tPlayerInt implements Plugin{
	...
	public function init(){
		...
		$ALLOW_BLOCK_DESTRUCTION = $cfg_defs["allow-block-destruction"];
		$ALLOW_BLOCK_PLACEMENT = $cfg_defs["allow-block-placement"];
		...
		if(!$ALLOW_BLOCK_DESTRUCTION) $this->api->addHandler("player.block.break", [$this, "blockBreak"], 777);
		if(!$ALLOW_BLOCK_PLACEMENT) $this->api->addHandler("player.block.place",   [$this, "blockPlace"], 777);
		if(!$ALLOW_CHAT) $this->api->addHandler("player.chat",                     [$this, "playerChat"], 777);
		if(!$ALLOW_PVP) $this->api->addHandler("player.interact",                  [$this, "punchMob"],   777);
		if(!$ALLOW_COMMANDS) $this->api->addHandler("console.command",             [$this, "cmdEvent"],   777);
