...
class Explosion{
	...
	public function explode(){
		$explosive= $this->source->name;
		console("[EXPLOSION]$explosive Detonation at $this->source");
		$radius = 2 * $this->size;
		...
		foreach($this->affectedBlocks as $xyz => $idm){
			...
			if ($explosive = " " && $explosive != "Creeper"){ //its tnt not a creeper
			...
			}else if (Explosion::$entityGriefing){  //&& $explosive = "Creeper"
				$dropz = $block->getDrops($this->air, $this->nullPlayer);
				if(is_array($dropz)){
					foreach($dropz as $drop){
						$server->api->entity->drop(new Position($x + 0.5, $y, $z + 0.5, $this->level), BlockAPI::getItem($drop[0], $drop[1], $drop[2])); //id, meta, count
			...
			//its likely a creeper but griefing is disabled
			}
			...
			if (!Explosion::$entityGriefing){ 
			if ($explosive == "Creeper"){
				//mob griefing is off, do nothing
				//console("$explosive exploded safely.");
			}else if ($explosive != "Creeper"){
				$this->level->fastSetBlockUpdate($x, $y, $z, 0, 0, true);
				//console("[INFO]$explosive Explosion damage at $this->source");
				}
			}else{ // Griefing is on
