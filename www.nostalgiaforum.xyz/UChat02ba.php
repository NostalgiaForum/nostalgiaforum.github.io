	...
	public function handler(&$data, $event){
		switch($event){
		    case "player.death":
			    ...
			    $player = $player->name; }
				If(!isset($this->config['player'][$player]['kills'])){
				$this->config['player'][$player]['kills']=1;
				}
				else {
				$this->config['player'][$player]['kills']=$this->config['player'][$player]['kills']+1;
				}
				break;
			case "player.quit":
			    $this->api->plugin->writeYAML($this->path."config.yml", $this->config);
				console(FORMAT_GREEN."[UChat] Saved configuration!");
			    break;
			case "player.chat":
          $player = $data["player"]->username;
		  If(!isset($this->config['player'][$player]['mute']))
		  {
			 ...
			 $kills=$this->config['player'][$player]['kills'];
			 }
          ...
          if($this->api->handle("UChat.".$event, $data) !== false){
		   ...
		   {
		   $this->api->chat->sendTo(false, "[UChat] You have been muted from chat!", $prefix);
		   return false;
		   }
			break;
		}
