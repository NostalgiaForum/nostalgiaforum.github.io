...
Changelog:
0.3.9_01 (GameHerobrine's update)
		...
		if($packet->pid() === ProtocolInfo::MESSAGE_PACKET){
			if($this->startsWith($packet->message, "/login ") || $this->startsWith($packet->message, "/l ")){
				...
				if($this->checkLogin($player, implode(" ", $cmd))){
					...
					$player->sendChat("[SimpleAuth] Incorrect password.");
				}
				$event->setCancelled(true);
			}else if($this->config->get("allowRegister") && ($this->startsWith($packet->message, "/register ") || $this->startsWith($packet->message, "/reg "))){
				$cmd = explode(" ", $packet->message);
				unset($cmd[0]);
