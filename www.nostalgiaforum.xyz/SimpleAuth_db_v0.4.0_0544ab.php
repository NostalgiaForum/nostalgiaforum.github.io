...
version=0.4.0_05
author=shoghicp/ArkQuark/GameHerobrine/realfreehij/Effently
class=SimpleAuth
apiversion=12,12.1
*/
...
Changelog:
0.4.0_05 (GameHerobrine)
* Fixed ability to execute commands when not logged in/registered
...
class SimpleAuth implements Plugin{
	private $api, $server, $config, $sessions, $database, $messages, $error, $badPasswordMessage, $shortPasswordMessage, $incorrectPassword, $successRegisterMessage, $successUnregisterMessage, $successAuthMessage, $registerMessage, $loginMessage, $joinMessage, $tries, $lastBroadcast = 0;
	...
	public function init(){
		...
		$this->messages = new Config($this->api->plugin->configPath($this)."messages.yml", CONFIG_YAML, [
			...
			"joinMessage" => "[SimpleAuth] This server uses SimpleAuth to protect your account."
		]);
		...
		$this->joinMessage = $this->messages->get("joinMessage");
		...
		if(file_exists($this->api->plugin->configPath($this)."players/")){
			console("[INFO] [SimpleAuth] Importing from old format to .db format...");
			$files = scandir($this->api->plugin->configPath($this)."players/");
			foreach($files as $path){
				if($path !== "." and $path !== ".."){
					$path = $this->api->plugin->configPath($this)."players/".$path;
					foreach(scandir($path) as $file){
						if($file !== "." and $file !== ".."){
							$test = new Config($path."/".$file, CONFIG_YAML);
							$this->addData(str_replace(".yml", "", $file), $test->get("registerdate"), $test->get("logindate"), $test->get("lastip"), $test->get("hash"));
							unlink($path."/".$file);
						}
					}
					rmdir($path);
			...
			rmdir($this->api->plugin->configPath($this)."players/");
		}
		...
		if($packet->pid() === ProtocolInfo::MESSAGE_PACKET){
			if($this->startsWith($packet->message, "/login ") || $this->startsWith($packet->message, "/l ")){
				...
				if($this->checkLogin($player, implode(" ", $cmd))){
					...
					if($this->tries[$player->iusername] > $this->config->get("maxTries")){
						$player->close("too many tries to log in");
					}else{
						$this->tries[$player->iusername] += 1;
	...
	public function eventHandler($data, $event){
		switch($event){
			case "player.quit":
				...
				unset($this->tries[$data->iusername]);
				break;
			case "player.connect":
			...
			case "player.spawn":
				if(!isset($this->sessions[$data->CID]) or $this->sessions[$data->CID] !== true){
					...
					$this->tries[$data->iusername] = 0;
					$data->blocked = true;
					$data->sendChat($this->joinMessage);
